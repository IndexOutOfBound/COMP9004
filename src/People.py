...
class People(object):
    ...
    def turn_towards_grain(self):
        ...
        best_left = np.sum(
            ...
            self.world.grains_distribution[min(self.world.WORLD_SIZE_X + self.axis_x - self.__vision, self.world.WORLD_SIZE_X) : , self.axis_y]
        )
        ...
        best_up = np.sum(
            ...
            self.world.grains_distribution[self.axis_x, min(self.world.WORLD_SIZE_Y, self.axis_y - self.__vision + self.world.WORLD_SIZE_Y) : ]
        )
        best_down = np.sum(
