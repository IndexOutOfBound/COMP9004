...
class People(object):
    ...
    def turn_towards_grain(self):
        ...
        best_left = np.sum(
            self.world.grains_distribution[
                max(0, self.axis_x-self.__vision) : self.axis_x, self.axis_y
                ]
        ) + np.sum(
            self.world.grains_distribution[
                min(self.world.WORLD_SIZE_X+self.axis_x-self.__vision, self.world.WORLD_SIZE_X):,
                self.axis_y
                ]
        )
        ...
        best_right = np.sum(
            self.world.grains_distribution[
                self.axis_x : min(self.__vision+self.axis_x, self.world.WORLD_SIZE_X), self.axis_y
                ]
        ) + np.sum(
            self.world.grains_distribution[
                : max(0, self.axis_x + self.__vision - self.world.WORLD_SIZE_X), self.axis_y
                ]
        )

        best_up = np.sum(
            self.world.grains_distribution[
                self.axis_x, min(0, self.axis_y-self.__vision) : self.axis_y] 
        ) + np.sum(
            self.world.grains_distribution[
                self.axis_x, 
        ...
        best_down = np.sum(
            self.world.grains_distribution[
                self.axis_x, self.axis_y : min(self.world.WORLD_SIZE_Y, self.axis_y+self.__vision)
                ]
        ) + np.sum(
            self.world.grains_distribution[
                self.axis_x, : max(0, self.axis_y + self.__vision - self.world.WORLD_SIZE_Y)
                ]
        )
    ...
    def reborn(self):
        ...
        self.__life_expectancy = np.random.randint(
